(function() {
    layui.use(['upload', 'form', 'element', 'table', "layer", "laydate", "util"], function() {
        ...
        function getBasSafetystandards(id){
            ...
            handleAjax('laws/getBasLawsregulations', 
            option,
             "GET").done(function(resp) {
                console.log(resp)
                document.querySelector('#lawname').value = resp.pageBean.lists[0].lawname;
                document.querySelector('#documentunit').value = resp.pageBean.lists[0].documentunit;
                document.querySelector('#lawtype').value = resp.pageBean.lists[0].lawtype;
        ...
        function uploadImg(){
            var uploadInst = upload.render({
                elem: '#selectImg' //绑定元素
                    ,
                url: base + "laws/addBasLawsregulations" //上传接口
                    ,
                auto: false,
                field:'accessory',
                headers: {
                ...
                data: {
                    ...
                    documentunit: function() {
                        return $('#documentunit').val();
                    },
                    lawtype: function() {
                        return $('#lawtype').val();
                    },
        ...
        function updateImg(id){
            var uploadInst = upload.render({
                elem: '#selectImg' //绑定元素
                    ,
                url: base + "laws/updateBasLawsregulations" //上传接口
                    ,
                field:'accessory',
        ...
        function updateInfo(id){
            handleAjax('laws/updateBasLawsregulations', { 
                id:id,
                lawname: $('#lawname').val(),
               
                documentunit: $('#documentunit').val(),
